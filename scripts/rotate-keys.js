...
const crypto_1 = require("../src/database/crypto");
// --- CLI argument parsing ---------------------------------------------------
const argv = process.argv.slice(2);
const dryRun = argv.includes('--dry-run');
const backupPathArg = argv.find((a) => a.startsWith('--backup-path='))?.split('=')[1];
const concurrency = parseInt(argv.find((a) => a.startsWith('--concurrency='))?.split('=')[1] ?? '10', 10);
const DB_PATH = path_1.default.join(process.cwd(), 'data', 'hayonero2.db');
// ---------------------------------------------------------------------------
...
async function main() {
    ...
    const currentVersion = (0, crypto_1.getCurrentKeyVersion)();
    console.log(`   Target key version: v${currentVersion}\n`);
    // --- Backup ----------------------------------------------------------------
    if (!dryRun) {
        const backupDir = backupPathArg ?? path_1.default.join(process.cwd(), 'data', 'backups');
        if (!fs_1.default.existsSync(backupDir))
            fs_1.default.mkdirSync(backupDir, { recursive: true });
        const ts = new Date().toISOString().replace(/[:.]/g, '-');
        const backupFile = path_1.default.join(backupDir, `hayonero2_pre-rotate_${ts}.db`);
        fs_1.default.copyFileSync(DB_PATH, backupFile);
        console.log(`💾 Backup created: ${backupFile}\n`);
    }
    // --- Load database ---------------------------------------------------------
    const SQL = await (0, sql_js_1.default)();
    const dbBuffer = fs_1.default.readFileSync(DB_PATH);
    ...
    const stmt = db.prepare('SELECT guild_id, data, iv, tag, key_version FROM server_settings');
    const records = [];
    while (stmt.step()) {
    ...
    await processInBatches(pending, concurrency, async (record) => {
        try {
            ...
            const settings = (0, crypto_1.decrypt)(record.guild_id, payload);
            const newPayload = (0, crypto_1.encrypt)(record.guild_id, settings);
            if (dryRun) {
                console.log(`  ✅ [DRY RUN] ${record.guild_id}: v${record.key_version} → v${newPayload.keyVersion}`);
            }
            else {
                db.run(`UPDATE server_settings
             SET data = ?, iv = ?, tag = ?, key_version = ?, updated_at = unixepoch()
           WHERE guild_id = ?`, [newPayload.data, newPayload.iv, newPayload.tag, newPayload.keyVersion, record.guild_id]);
                console.log(`  ✅ ${record.guild_id}: v${record.key_version} → v${newPayload.keyVersion}`);
            }
            rotated++;
        }
        catch (err) {
            console.error(`  ❌ ${record.guild_id}: rotation failed —`, err.message);
            failed++;
        }
    });
    // --- Save database ---------------------------------------------------------
    if (!dryRun && rotated > 0) {
        ...
        fs_1.default.writeFileSync(DB_PATH, Buffer.from(exported));
